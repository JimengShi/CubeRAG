...
def decompose_query_along_cultural_product(query, dimensions):
    ...
    try:
        ...
        for ent in detected_ents.list_of_queries:
            ...
            embed_string(target_str=ent.query_content, emb_model=model, dict_path='ent2emb/ent2emb_lveval_product.pkl', )
            # embed_string(target_str=ent.query_content, emb_model=model, dict_path='ent2emb/ent2emb_lveval.pkl', )
        return cells
...
def decompose_query_along_person(query, dimensions):
    ...
    try:
        ...
        for ent in detected_ents.list_of_queries:
            cells[ent.query_dimension].append(ent.query_content)
            embed_string(target_str=ent.query_content, emb_model=model, dict_path='ent2emb/ent2emb_lveval_person.pkl', )
            # embed_string(target_str=ent.query_content, emb_model=model, dict_path='ent2emb/ent2emb_lveval.pkl', )
        return cells
...
def decompose_query_along_location(query, dimensions):
    ...
    system_prompt = (
        ...
        f"Note that each of your query should be an entity or a short phrase and its associated dimension.\n\n"
        f"Example Input:\n"
        f"Question: Are Marufabad and Nasamkhrali both located in the same country? Where is the Havard University?\n"
        f"Example Output:\n"
        f"Query 1:\n"
        f"query_dimension: 'location_organization'; query_content: 'Marufabad';\n"
        f"Query 2:\n"
        f"query_dimension: 'location_organization'; query_content: 'Nasamkhrali';\n"
        f"Query 3:\n"
        f"query_dimension: 'location_type'; query_content: 'country';\n"
        f"Query 4:\n"
        f"query_dimension: 'location_relation'; query_content: 'located in';\n"
        f"Query 5:\n"
        f"query_dimension: 'location_organization'; query_content: 'Havard University?';\n"
    )
    ...
    class Query(BaseModel):
        ...
        query_dimension: Literal['location_organization', 'location_type', 'location_relation'] = Field(
            ...,
            title='Dimension of the entity or phrase to query documents'
        )
    ...
    try:
        ...
        for ent in detected_ents.list_of_queries:
            cells[ent.query_dimension].append(ent.query_content)
            embed_string(target_str=ent.query_content, emb_model=model, dict_path='ent2emb/ent2emb_lveval_location.pkl', )
            # embed_string(target_str=ent.query_content, emb_model=model, dict_path='ent2emb/ent2emb_lveval.pkl', )
        return cells
