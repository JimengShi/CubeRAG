...
from utils.cube_router_hotpotqa import route_query_to_retrieve_docs_hotpotqa

from utils.merge_return_doc import merge_return_doc_idx
from utils.load_data import load_qa, load_corpus

corpus_data_path = 'corpus/hotpotqa/hotpotqa_corpus_with_index.json'
...
def solve_multi_hop_query_iterative_hotpotqa(model_type, original_query, corpus, k):
    ...
    while current_query:
        ...
        if current_query == "FINAL ANSWER" or current_query.startswith("FINAL ANSWER") or step > 5:
            final_prompt = f"""
        ...
        return_docs, return_doc_idxs = route_query_to_retrieve_docs_hotpotqa(current_query, corpus, k)
        ...
        if the original query asks The fictional private detective that appears in "The Adventure of the Seven Clocks" what written by whom?, then you should include who is fictional private detective and continue reasoning based on the intermidiate answers.
