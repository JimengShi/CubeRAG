...
def decompose_query_along_cultural_product(query, dimensions):
    ...
    system_prompt = (
        ...
        f"cultural_product_name dimension can be specific names/entities/phrases/terms of film, movie, song, magzine, band.\n"
        f"cultural_product_type dimension can be types of cultural_products, such as film, movie, song, magzine.\n"
        f"action dimension can be any actions that describe the entities/phrases/terms in the above cultural_product dimension.\n"
        ...
        f"Query 3:\n"
        f"query_dimension: 'action'; query_content: 'directed';\n"
        f"Query 4:\n"
        f"query_dimension: 'action'; query_content: 'released';\n"
    )
    ...
    class Query(BaseModel):
        ...
        query_dimension: Literal['cultural_product_name', 'cultural_product_type', 'action'] = Field(
            ...,
    ...
    try:
        ...
        for ent in detected_ents.list_of_queries:
            ...
            embed_string(target_str=ent.query_content, emb_model=model, dict_path='ent2emb/ent2emb_wikimultihop.pkl', )
        return cells
...
def decompose_query_along_person(query, dimensions):
    ...
    system_prompt = (
        ...
        f"role dimension can be director, spouse, child, mother, father, grandfather, performer, composer, father-in-law, stepmother, mother-in-law, stepmother, child-in-law, sibling-in-law, husband, wife, uncle, founder, paternal grandmother, maternal grandfather. Feel free to include more.\n"
        f"behavior dimension can be die on, born on, birth, married to, study, work at, work as, death, located at entities/phrases/terms. Feel free to include more.\n"
        f"nationality dimension can be such as come from, is from, the country is. Feel free to include more.\n"
...
def decompose_query_along_location(query, dimensions):
    ...
    system_prompt = (
        ...
        f"location_name dimension can be specific names/entities/phrases/terms of locations.\n"
        f"location_type dimension can be street, village, city, county, state, country. Feel free to include more.\n"
