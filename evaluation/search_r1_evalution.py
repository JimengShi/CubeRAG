import json
import argparse
import pandas as pd
from tqdm import tqdm
import sys
import os
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), "..")))


# ================ Load Dataset ================
def load_dataset(data_path):
    if data_path.endswith(".json"):
        with open(data_path, "r") as f:
            return json.load(f)
    elif data_path.endswith(".csv"):
        df = pd.read_csv(data_path)
        return df.to_dict(orient="records")  # Converts to list of dicts
    else:
        raise ValueError("Unsupported file format. Use JSON or CSV.")
    

# ================ Evaluation Metrics ================
from utils.metric import exact_match, f1_score, bleu_score, rouge_score, semantic_score, em_score, em_check


# ================ Argument Parser ================
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate QA model performance.")
    parser.add_argument("--data", type=str, default="wikimultihop", choices=['hurricane', 'geography', 'aging_dam', 'hydrology', 'chemistry', 'legalbench', 'scifact', 'wikimultihop'], help="data set.")
    parser.add_argument("--model", type=str, choices=["gpt-4", "gpt-4o", "gpt-3.5-turbo", "gpt-4o-mini", "search_r1"], help="Select llm to get answer.")
    parser.add_argument("--metric", type=str, default="all", choices=["em", "f1", "bleu", "rouge", "semantic", "all"], help="Evaluation metric: one score or multiple scores.")
    return parser.parse_args()


# ================ Main Function ================
def main():
    args = parse_args()

    # llm_output_dir = f"output/{args.data}/search_r1/2wiki_results_top3_max5_processed.json"
    llm_output_dir = f"output/{args.data}/search_r1/2wiki_results_top5_max5_processed.json"

    # Load dataset
    qa_samples = load_dataset(llm_output_dir)

    # Evaluation
    print("\n--- QA Evaluation ---\n")

    # Initialize evaluation metrics
    total_em, total_f1, total_bleu, total_rouge, total_semantic, total_semantic = 0, 0, 0, 0, 0, 0

    for i, sample in enumerate(qa_samples):
        print(f"Sample: {i+1}")
        gold_answer = sample["gold_answer"]  
        predicted_answer = sample["answer"]

        # print(f">>> Predicted: {predicted_answer}")
        # print(f">>> Ground Truth: {gold_answer}")

        
        # Calculate metrics of each sample based on the selected option
        if args.metric in ["em", "all"]:
            # em = exact_match(predicted_answer, gold_answer)
            # em = em_score(predicted_answer, gold_answer) 
            em = em_check(predicted_answer, gold_answer) 
            total_em += em
            # print(f"Exact Match: {em:.2f}")
        if args.metric in ["f1", "all"]:
            f1 = f1_score(predicted_answer, gold_answer)
            total_f1 += f1
            # print(f"F1 Score: {f1:.2f}")
        if args.metric in ["bleu", "all"]:
            bleu = bleu_score(predicted_answer, gold_answer)
            total_bleu += bleu
            # print(f"Bleu Score: {bleu:.2f}")
        if args.metric in ["rouge", "all"]:
            rouge = rouge_score(predicted_answer, gold_answer)
            total_rouge += rouge
            # print(f"ROUGE Score: {rouge:.2f}")
        if args.metric in ["semantic", "all"]:
            semantic = semantic_score(predicted_answer, gold_answer)
            total_semantic += semantic
            print(f"Semantic Score: {semantic:.2f}")


    # Averaging Summary Information
    num_samples = len(qa_samples)
    if total_em:
        print(f"Total samples: {num_samples}, average EM: {total_em / num_samples:.4f}")
    if total_f1:
        print(f"Total samples: {num_samples}, average F1: {total_f1 / num_samples:.4f}")
    elif total_semantic:
        print(f"Total samples: {num_samples}, average Semantic: {total_semantic / num_samples:.4f}")
    # print(f"Total samples: {num_samples}, average EM: {total_em / num_samples:.4f}, average F1: {total_f1 / num_samples:.4f}, average BLEU: {total_bleu / num_samples:.4f}, average ROUGE: {total_rouge / num_samples:.4f}, average Semantic: {total_semantic / num_samples:.4f}")


if __name__ == "__main__":
    main()
