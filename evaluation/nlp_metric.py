...
# ================ Evaluation Metrics ================
from utils.metric import exact_match, f1_score, bleu_score, rouge_score, semantic_score, em_score, em_check, subem_check


# ================ Argument Parser ================
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate QA model performance.")
    parser.add_argument("--data", type=str, required=True, default="wikimultihop", choices=['hurricane', 'geography', 'aging_dam', 'hydrology', 'chemistry', 'legalbench', 'scifact', 'wikimultihop'], help="data set.")
    parser.add_argument("--model", type=str, choices=["gpt-4o", "llama3", "gpt-4o-mini", "search_r1"], help="Select llm to get answer.")
    parser.add_argument("--retriever", type=str, default="hypercube", choices=['hypercube', 'semantic', 'union', 'hipporag', 'hipporag2', 'lightrag', 'contriever', 'graphrag', 'bm25', 'none'], help="Retrieval methods.")
...
def main():
    ...
    elif args.retriever == 'graphrag':
        llm_output_dir = f"output/{args.data}/{args.model}/llm_output_graphrag.json"
    elif args.retriever == 'hipporag':
        llm_output_dir = f"output/{args.data}/{args.model}/llm_output_hipporag.json"
    elif args.retriever == 'hipporag2':
        llm_output_dir = f"output/{args.data}/{args.model}/llm_output_hipporag_k5_responses.json"
    elif args.retriever == 'lightrag':
        llm_output_dir = f"output/{args.data}/{args.model}/llm_output_lightrag.json"
    elif args.data == 'legalbench':
        llm_output_dir = f"output/{args.data}/{args.model}/llm_output_{args.retriever}_k5.json"
    elif args.data == 'wikimultihop':
        llm_output_dir = f"output/{args.data}/llm_output_{args.model}_k5_responses.json"
    else:
        llm_output_dir = f"output/{args.data}/{args.model}/llm_output_{args.retriever}.json"
    ...
    total_em, total_f1, total_bleu, total_rouge, total_semantic, total_semantic = 0, 0, 0, 0, 0, 0

    for i, sample in enumerate(qa_samples):
        print(f"Sample: {i+1}")
        if args.retriever == "hipporag" or args.retriever == "hipporag2":
            gold_answer = sample["gold_answers"]
        elif args.data == "legalbench" or args.data == "scifact":
        ...
        if args.metric in ["em", "all"]:
            ...
            em = em_score(predicted_answer, gold_answer) 
            # em = em_check(predicted_answer, gold_answer) 
            # em = subem_check(predicted_answer, gold_answer) 
            total_em += em
    ...
    if total_em:
        print(f"Total samples: {num_samples}, average EM: {total_em / num_samples:.4f}")
    if total_f1:
        print(f"Total samples: {num_samples}, average F1: {total_f1 / num_samples:.4f}")
    elif total_semantic:
        print(f"Total samples: {num_samples}, average Semantic: {total_semantic / num_samples:.4f}")
