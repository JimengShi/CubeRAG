...
from utils.decompose_musique import decompose_query_along_politics
import time
...
dimensions = ['politics']
hypercube = {'politics': defaultdict(list)}
    ...
    with open(f'hypercube/musique/{dimension}.txt') as f:
        ...
        for i, line in tqdm(enumerate(readin), total=len(readin), desc=f"{dimension}"):
            ...
            for k in tmp:
                ...
                embed_string(target_str=k, emb_model=model, dict_path='ent2emb/ent2emb_musique_politics.pkl', )
    ...
    for key, v in cells.items():        
        for vv in v:
            ...
            if vv in hypercube[key]:
                ...
                embed_string(target_str=vv, emb_model=model, dict_path='ent2emb/ent2emb_musique_politics.pkl')
...
def retriever_hypercube_politics(query, corpus, k):
    # corpus = read_corpus()
    
    cells = decompose_query_along_politics(query, dimensions)
