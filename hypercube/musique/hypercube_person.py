...
from utils.decompose_musique import decompose_query_along_person
import time
...
dimensions = ['person', 'role_occupation', 'behavior', 'nationality']
hypercube = {'person': defaultdict(list),
             'role_occupation': defaultdict(list),
    ...
    with open(f'hypercube/musique/{dimension}.txt') as f:
        ...
        for i, line in tqdm(enumerate(readin), total=len(readin), desc=f"{dimension}"):
            ...
            for k in tmp:
                ...
                embed_string(target_str=k, emb_model=model, dict_path='ent2emb/ent2emb_musique_person.pkl', )
    ...
    for key, v in cells.items():        
        for vv in v:
            ...
            if vv in hypercube[key]:
                ...
                embed_string(target_str=vv, emb_model=model, dict_path='ent2emb/ent2emb_musique_person.pkl', )
...
def retriever_hypercube_person(query, corpus, k):
    # corpus = read_corpus()
    

    cells = decompose_query_along_person(query, dimensions)
