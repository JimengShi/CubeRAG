...
from utils.decompose_musique import decompose_query_along_cultural_product
import time
...
def load_ent2emb(dict_path):
    if os.path.exists(dict_path):
        with open(dict_path, "rb") as f:
            return pickle.load(f)
    else:
        print("No saved dictionary found, initialized a new dict.")
        return {}

def save_ent2emb(dict_path, ent2emb):
    with open(dict_path, "wb") as f:
        pickle.dump(ent2emb, f, protocol=pickle.HIGHEST_PROTOCOL)
...
# ================ read corpus ================
def read_corpus():
    with open('corpus/musique/musique_corpus_with_index.json', 'r', encoding='utf-8') as f:
        data = json.load(f)
...
dimensions = ['cultural_product_name', 'cultural_product_type', 'action_relation']
...
# dict_path='ent2emb/ent2emb_musique_product.pkl'
# ent2emb = load_ent2emb(dict_path)


for dimension in dimensions:
    with open(f'hypercube/musique/{dimension}.txt') as f:
        readin = f.readlines()
        # readin = readin[:1000]
        for i, line in tqdm(enumerate(readin), total=len(readin), desc=f"{dimension}"):
            ...
            for k in tmp:
                ...
                # embed_string(target_str=k, emb_model=model, dict_path='ent2emb/ent2emb_musique.pkl')
                embed_string(target_str=k, emb_model=model, dict_path='ent2emb/ent2emb_musique_product.pkl')
                # if k not in ent2emb:
                #     ent2emb[k] = model.encode('query: ' + k, normalize_embeddings=True)

# # Save once at the end
# save_ent2emb(dict_path, ent2emb)
    ...
    for key, v in cells.items():        
        for vv in v:
            ...
            if vv in hypercube[key]:
                ...
                # embed_string(target_str=vv, emb_model=model, dict_path='ent2emb/ent2emb_musique.pkl')
                embed_string(target_str=vv, emb_model=model, dict_path='ent2emb/ent2emb_musique_product.pkl')
                vv_emb = ent2emb[vv]
