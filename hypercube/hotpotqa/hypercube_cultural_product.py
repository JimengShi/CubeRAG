...
from utils.decompose_hotpotqa import decompose_query_along_cultural_product
...
dimensions = ['cultural_product_media', 'cultural_product_type', 'action_relation', 'date', 'location_name', 'person_name', 'company_organization']
hypercube = {'cultural_product_media': defaultdict(list),
             ...
             'company_organization': defaultdict(list)}
    ...
    with open(f'hypercube_new/hotpotqa/{dimension}.txt') as f:
        ...
        for i, line in tqdm(enumerate(readin), total=len(readin), desc=f"{dimension}"):
            ...
            for k in tmp:
                ...
                # embed_string(target_str=k, emb_model=model, dict_path='ent2emb/ent2emb_hotpotqa_product.pkl')
    ...
    for key, v in cells.items():        
        for vv in v:
            ...
            if vv in hypercube[key]:
                ...
                # embed_string(target_str=vv, emb_model=model, dict_path='ent2emb/ent2emb_hotpotqa_product.pkl')
