...
from utils.decompose_hotpotqa import decompose_query_along_plant_animal
...
dimensions = ['plant_animal', 'date', 'location_name']
hypercube = {'plant_animal': defaultdict(list),
    ...
    with open(f'hypercube_new/hotpotqa/{dimension}.txt') as f:
        ...
        for i, line in tqdm(enumerate(readin), total=len(readin), desc=f"{dimension}"):
            ...
            for k in tmp:
                ...
                # embed_string(target_str=k, emb_model=model, dict_path='ent2emb/ent2emb_hotpotqa_plant_animal.pkl', )
    ...
    for key, v in cells.items():        
        for vv in v:
            ...
            if vv in hypercube[key]:
                ...
                # embed_string(target_str=vv, emb_model=model, dict_path='ent2emb/ent2emb_hotpotqa_plant_animal.pkl')
...
def retriever_hypercube_plant_animal(query, corpus, k):
    # corpus = read_corpus()
    
    cells = decompose_query_along_plant_animal(query, dimensions)
