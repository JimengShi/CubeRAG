...
from utils.decompose_hotpotqa import decompose_query_along_person
...
dimensions = ['person_name', 'role_occupation', 'person_behavior', 'nationality', 'relationship', 'date', 'location_name']
hypercube = {'person_name': defaultdict(list),
             'role_occupation': defaultdict(list),
             'person_behavior': defaultdict(list),
    ...
    with open(f'hypercube_new/hotpotqa/{dimension}.txt') as f:
        ...
        for i, line in tqdm(enumerate(readin), total=len(readin), desc=f"{dimension}"):
            ...
            for k in tmp:
                ...
                # embed_string(target_str=k, emb_model=model, dict_path='ent2emb/ent2emb_hotpotqa_person.pkl', )
    ...
    for key, v in cells.items():        
        for vv in v:
            ...
            if vv in hypercube[key]:
                ...
                # embed_string(target_str=vv, emb_model=model, dict_path='ent2emb/ent2emb_hotpotqa_person.pkl')
