...
from utils.decompose_lveval import decompose_query_along_location
import time
...
# def embed_string(target_str, emb_model, dict_path='ent2emb/ent2emb_wikimultihop_location.pkl'):
...
# ================ read corpus ================
def read_corpus():
    with open('corpus/lveval/lveval_corpus_with_index.json', 'r', encoding='utf-8') as f:
        data = json.load(f)
...
dimensions = ['location_organization', 'location_type', 'location_relation']
hypercube = {'location_organization': defaultdict(list),
             'location_type': defaultdict(list),
             'location_relation': defaultdict(list)}

model = SentenceTransformer('intfloat/e5-base-v2')
for dimension in dimensions:
    with open(f'hypercube/lveval/{dimension}.txt') as f:
        readin = f.readlines()
        # readin = readin[:10000]
        for i, line in tqdm(enumerate(readin), total=len(readin), desc=f"{dimension}"):
            ...
            for k in tmp:
                ...
                embed_string(target_str=k, emb_model=model, dict_path='ent2emb/ent2emb_lveval_location.pkl', )
                # embed_string(target_str=k, emb_model=model, dict_path='ent2emb/ent2emb_lveval.pkl')
...
def get_docs_from_cells(cells, top_k):
    ...
    for key, v in cells.items():        
        for vv in v:
            ...
            # else:
            #     embed_string(target_str=vv, emb_model=model, dict_path='ent2emb/ent2emb_lveval_location.pkl')
            #     # embed_string(target_str=vv, emb_model=model, dict_path='ent2emb/ent2emb_lveval.pkl')
            #     vv_emb = ent2emb[vv]
...
def retriever_hypercube_location(query, corpus, k):
    # corpus = read_corpus()
    
    cells = decompose_query_along_location(query, dimensions)
