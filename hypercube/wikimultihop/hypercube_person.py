...
from utils.decompose import decompose_query_along_person
import time
...
dimensions = ['person', 'role', 'behavior', 'nationality']
    ...
    with open(f'hypercube/wikimultihop/{dimension}.txt') as f:
        ...
        for i, line in tqdm(enumerate(readin), total=len(readin), desc=f"{dimension}"):
            ...
            for k in tmp:
                ...
                # embed_string(target_str=k, emb_model=model, dict_path='ent2emb/ent2emb_wikimultihop_person.pkl', )
    ...
    for key, v in cells.items():        
        for vv in v:
            ...
            if vv in hypercube[key]:
                ...
                embed_string(target_str=vv, emb_model=model, dict_path='ent2emb/ent2emb_wikimultihop_person.pkl', )
                vv_emb = ent2emb[vv]
